...
public class Buscaminas extends Application {
  ...
  public static void jugarPartida () {
    ...
    for (int i=0; i < numeroMinas; i++) {
      ...
      } while (casilla[minaColumna][minaFila].isEsMina());
    ...
    if (casilla[i][j].isEsMina()){
      ...
      for (int a = 0; a < columnas; a++) {
        for (int b = 0; b < filas; b++) {
          if (casilla[a][b].isEsMina()) {
            ImageView imageView = new ImageView();
            imageView.setPreserveRatio(true);
            imageView.setFitWidth(25);
            imageView.setImage(new Image("file:src/ejercicios/gui/img/mina.png"));
            casilla[a][b].setGraphic(imageView);
            casilla[a][b].setStyle("-fx-background-color: #fb0000;" + "-fx-font-color: #ffffff;");
          }
  ...
  public static void marcarMina (int i, int j) {
    ...
    switch (casilla[i][j].getEstado()){
      case 0:
        imageView.setImage(new Image("file:src/ejercicios/gui/img/bandera.png"));
        casilla[i][j].setStyle("-fx-background-color: #faac03;");
        casilla[i][j].cambiarEstado();
        break;
      case 1:
        imageView.setImage(new Image("file:src/ejercicios/gui/img/interrogacion.png"));
